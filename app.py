...
# 1. ANAHTARIN BOZULMASI İMKANSIZ FORMATI
# Satırları liste olarak tanımlayıp, temiz bir şekilde birleştiriyoruz.
key_parts = [
    "-----BEGIN PRIVATE KEY-----",
    ...
    "nc4mcUVyySAu8Y9vQnQs6KOF2eogKTtBO9/FK7ZnvGGFdyudQnsj67sHWILvJ3TGt",
...
# Alt alta birleştiriyoruz
PRIVATE_KEY = "\n".join(key_parts) + "\n"

# 2. SERVİS HESABI BİLGİLERİ
...
try:
    ...
    if not df.empty:
        ...
        st.subheader("🛡️ Kahraman Tablosu")
        ...
        if st.button(f"✨ {secilen}'e XP Tanımla"):
            # Satır bulma (index + başlık(1) + Google 1-index(1) = +2)
            row_idx = df.index[df['ogrenci'] == secilen].tolist()[0] + 2
            mevcut_xp = int(df.loc[df['ogrenci'] == secilen, 'xp'].values[0])
            
            # Google Sheet'te XP sütununu (genelde 2. sütun) güncelle
            worksheet.update_cell(row_idx, 2, mevcut_xp + miktar)
            
            st.balloons()
            st.success(f"Başarılı! {secilen} için XP güncellendi.")
            st.rerun()
