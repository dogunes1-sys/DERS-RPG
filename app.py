import streamlit as st
import gspread
from google.oauth2.service_account import Credentials
import pandas as pd

# 1. TÜM BİLGİLERİ KODUN İÇİNE ALDIK (HATA PAYI SIFIR)
SERVICE_ACCOUNT_INFO = {
    "type": "service_account",
    "project_id": "dersrpg",
    "private_key_id": "d4e4b87ab157fd2dd9a8f2aea0ea1bed5cefbe41",
    "private_key": "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n",
    "client_email": "ders-rpg-bot@dersrpg.iam.gserviceaccount.com",
    "token_uri": "https://oauth2.googleapis.com/token",
}

# Sayfa Yapısı
st.set_page_config(page_title="Ders RPG Kontrol", layout="wide")
st.title("🎮 Ders RPG Kontrol Paneli")

def get_worksheet():
    scopes = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    creds = Credentials.from_service_account_info(SERVICE_ACCOUNT_INFO, scopes=scopes)
    client = gspread.authorize(creds)
    # Sheet'i aç
    sh = client.open_by_key("1NJob3RNvMZ43_JlG1hnaZmnF_I3bUW3BtW9bsNx6kB8")
    return sh.get_worksheet(0)

try:
    worksheet = get_worksheet()
    data = worksheet.get_all_records()
    df = pd.DataFrame(data)

    if not df.empty:
        st.success("✅ Bağlantı başarıyla sağlandı!")
        st.dataframe(df, use_container_width=True)

        st.divider()
        st.subheader("🧙‍♂️ Kahraman Yönetimi")
        
        col1, col2 = st.columns(2)
        with col1:
            secilen = st.selectbox("Bir Kahraman Seç:", df["ogrenci"].tolist())
        with col2:
            miktar = st.number_input("Eklenecek XP:", min_value=1, value=10, step=5)

        if st.button(f"✨ {secilen}'e XP Tanımla"):
            row_idx = df.index[df['ogrenci'] == secilen].tolist()[0] + 2
            mevcut_xp = int(df.loc[df['ogrenci'] == secilen, 'xp'].values[0])
            worksheet.update_cell(row_idx, 2, mevcut_xp + miktar)
            st.balloons()
            st.success(f"{secilen} artık {mevcut_xp + miktar} XP!")
            st.rerun()
    else:
        st.warning("Veritabanı boş görünüyor.")

except Exception as e:
    st.error(f"❌ Beklenmedik Hata: {e}")

if st.sidebar.button("🔄 Verileri Yenile"):
    st.rerun()
